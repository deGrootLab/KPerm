        ...
        for layer in sf_layer[atomtype].keys():
            for info in sf_layer[atomtype][layer].keys():
                (
                    ...
                    sf_layer[atomtype][layer][info][i_oppositeTo0],
                ) = (
                    sf_layer[atomtype][layer][info][i_oppositeTo0],
                    sf_layer[atomtype][layer][info][1],
                )
...
def detectSF_old(coor, quiet=False, o_cutoff=5, og1_cutoff=7.0, allRes=False):
    ...
    if allRes:
        ...
        protein_o = u.select_atoms(
            f"protein and name O and (resname GLY VAL TYR THR PHE ILE)", 
            updating=False
        )
    ...
    sf_o_resid = [i for chain in sf_o_resid[:] for i in chain]
    sf_o = u.select_atoms(
        f"protein and name O and resid {' '.join([str(i) for i in sf_o_resid])}",
        updating=False,
    )
    sf_ca = u.select_atoms(
        f"protein and name CA and resid {' '.join([str(i) for i in sf_ca_resid])}",
        updating=False,
    )
    ...
    sf_og1_indices = sf_og1_indices[n_neighbors > 2]
    sf_og1 = u.select_atoms(
        f"protein and name OG1 and index {' '.join([str(i) for i in sf_og1_indices])}",
        updating=False,
    )
    sf_o = u.select_atoms(
        f"protein and index  {' '.join([str(atom.ix) for atom in sf_o+sf_og1])}",
        updating=False,
    )
    ...
    for sf_atom, name in zip([sf_o, sf_ca], ["O", "CA"]):
        ...
        for atom in sf_atom:
            if layer_id not in sf_layer[name].keys():
                ...
                sf_layer[name][layer_id]["name"].append(atom.name)
            layer_id = layer_id - \
                1 if layer_id != 0 else (len(sf_atom) // 4 - 1)
    ...
    sf_ca_0_pos = u.select_atoms("all").positions[sf_layer["CA"][0]["idx"]]
    i_oppositeTo0 = np.linalg.norm(
        sf_ca_0_pos - sf_ca_0_pos[0], axis=1).argmax()
        ...
        for layer in sf_layer[atomtype].keys():
            ...
            if atomtype == "O" and not quiet:
                for chain_id, idx in enumerate(sf_layer[atomtype][layer]["idx"]
                    ...
                    print(f"{idx}\t{layer}\t{chain_id}\t{resid}\t{resname}\
                          \t{name}")
    ...
    def load(self, results_loc=None, inputName="kperm"):
        ...
        for result_loc in self.results_loc:
            ...
            log_path = os.path.abspath(
                os.path.join(result_loc, inputName + ".log"))
...
def run(
    ...
    for ts in u.trajectory:
        if CADistance:
            ...
            ca_d_diag[ts.frame] = computeSFAtomDistance(
                ts.positions, sf_ca_idx)

        flips[ts.frame] = checkFlips(ts.positions, sf_o_idx)

        k_occ = findBound(
            ts.positions, k_idx, sf_o_idx, BScenter_cutoff=BScenter_cutoff
        )
        w_occ = findBound(
            ts.positions, water_idx, sf_o_idx, BScenter_cutoff=BScenter_cutoff
        )
    ...
    print("")
    occupancy[: len(k_occupancy)], double_occ = computeOccupancy_6BS(
...
frames. Check log file for details."
    ...
    if "jump" in perm_count:
        jumps[: len(k_occupancy) -
              1] = computeJumps_6BS(k_occupancy, w_occupancy)
